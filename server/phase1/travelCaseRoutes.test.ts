...
function fakeStore(): SupabaseTravelCaseStore {
  return {
    ...
    updateComponent: vi.fn().mockResolvedValue({ ...detail, version: 1 }),
...
describe("membership-authorized travel-case routes", () => {
  ...
  it("requires a response summary when resubmitting returned information", async () => {
    const store = fakeStore(); const base = await appServer(store);
    const response = await fetch(`${base}/api/v1/travel-cases/${caseId}/submission`, {
      method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({
        expectedStatus: "information_required", expectedVersion: 3,
        idempotencyKey: "00000000-0000-4000-8000-000000000041", attestation: true,
        caseType: "corporate", travellerUserId: "00000000-0000-4000-8000-000000000013",
        startDate: "2026-10-01", endDate: "2026-10-02", destination: { city: "Suva", country: "Fiji" },
        funding: { costCentreId: "CC-1", purchaseOrderRequired: false }, requiredComponentTypes: ["flight"],
      }),
    });
    expect(response.status).toBe(422);
    expect(store.submit).not.toHaveBeenCalled();
  ...
  it("claims coordinator review through a versioned idempotent command", async () => {
    ...
    const payload = { expectedVersion: 1, idempotencyKey: "00000000-0000-4000-8000-000000000050" };
    const response = await fetch(`${base}/api/v1/travel-cases/${caseId}/review-assignment`, {
      method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(payload),
    });
    expect(response.status).toBe(200);
    expect(store.claimReview).toHaveBeenCalledWith(expect.objectContaining({ role: "coordinator" }), caseId, payload);
  });

  it("creates a structured information request", async () => {
    const store = fakeStore(); const base = await appServer(store, true, [{ id: membershipId, organisationId, status: "active", role: "coordinator" }]);
    const payload = { expectedVersion: 2, idempotencyKey: "00000000-0000-4000-8000-000000000051", reason: "Please confirm the revised travel dates", requestedFields: ["startDate", "endDate"], dueDate: "2026-09-10" };
    const response = await fetch(`${base}/api/v1/travel-cases/${caseId}/information-requests`, {
      method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(payload),
    });
    expect(response.status).toBe(201);
